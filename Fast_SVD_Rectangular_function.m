function [U_sorted, D_sorted, V_sorted] = Fast_SVD_Rectangular_function(K_RHS, K_LHS, n, k, number_of_passes, method)

%     function y = K(x, t_flag)
%         if t_flag == 0
%             y = K_RHS(x);
%         else
%             y = K_LHS(x);
...
%     end
    ...
    K = @(x, t_flag) t_flag * K_LHS(x) + (1 - t_flag)*K_RHS(x');

%     K(ones(n, 1), 0)

if strcmp(method, 'Lanczos') == 1

    [U_sorted, D_small, V_sorted] = svds(K, [n n], k);
    
    [D_sorted, indices_eval] = sort(diag(D_small(1:k, 1:k)), 'descend');
    
    U_sorted = U_sorted(:, indices_eval);
    V_sorted = V_sorted(:, indices_eval);


elseif strcmp(method, 'Halko') == 1
...
%%% Using Algorithm 5.1 from Tropp
